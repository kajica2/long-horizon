/**
 * render-7h-sheet.ts — 7-hour progress contact sheet.
 *
 * Shows 6 panels:
 *   1. SHIMMER (audio → particles)
 *   2. JAN 2026 (planetary → cosmic filaments)
 *   3. SAND TRAVELER (Tarbell port)
 *   4. DE JONG (Tarbell port)
 *   5. KEPLER BIRTH CHART (3D wheel)
 *   6. CAGE BIRTH CHART (3D wheel)
 */
...
const TILES = [
  { file: "tmp/renders/demo-demo-shimmerwav.png", label: "SHIMMER", subtitle: "Audio · Flow Field Meditation · audio-reactive" },
  { file: "tmp/renders/polaroid-planetary-jan2026.png", label: "JAN 15, 2026", subtitle: "Planetary · Cosmic Filaments · curl noise" },
  { file: "tmp/renders/sand-tarbell-port.png", label: "SAND TRAVELER", subtitle: "Tarbell 2004 · 200 cities · sand accumulation" },
  { file: "tmp/renders/dejong-rich.png", label: "DE JONG", subtitle: "Tarbell 2004 · 4K travelers · de Jong map" },
  { file: "tmp/renders/birth-chart-kepler.png", label: "KEPLER · 1571", subtitle: "Birth chart · 3D wheel · Placidus houses" },
  { file: "tmp/renders/birth-chart-cage.png", label: "CAGE · 1912", subtitle: "Birth chart · 3D wheel · aspect lines" },
];
...
async function main() {
  ...
  ctx.fillText("Stages 0–6 done  ·  5 living systems  ·  16 seed artworks  ·  80 tests  ·  audio-reactive + 3D birth chart", PADDING, 130);
  ...
  ctx.fillText("Top row: audio + planetary genomes.   Middle: classic Tarbell ports.   Bottom: 3D birth charts (Placidus).", PADDING, H - PADDING);

  const outPath = path.resolve("tmp/renders/7h-sheet.png");
  await fs.mkdir(path.dirname(outPath), { recursive: true });
  const buffer = canvas.toBuffer("image/png");
  await fs.writeFile(outPath, buffer);
  console.log(`[7h] wrote ${(buffer.length / 1024).toFixed(0)}KB → ${outPath}`);
}
